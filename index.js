...
function cumlelereDonustur(dizi, ayrac = "") {
  ...
  let yeniDizi = [];
  dizi.map((dizi) => {
    return yeniDizi.push(dizi.join(ayrac));
  });
  return yeniDizi;
}
...
function paragrafOlustur(dizi, cb1, cb2) {
  let yeniDizi = cb2(dizi, " ");
  yeniDizi = yeniDizi.filter((eleman, index) => {
    return [1, 3, 5, 7, 9].includes(index);
  });
  const paragraf = yeniDizi.reduce((acc, sentence) => cb1(acc, sentence), "");
  return paragraf;
}
...
function emojileriDonustur(mesaj, nesne) {
  for (let shrt in nesne) {
    mesaj = mesaj.replaceAll(shrt, nesne[shrt]);
    mesaj = mesaj.replaceAll(shrt.toUpperCase(), nesne[shrt]);
  }
  return mesaj;
}
console.log(
